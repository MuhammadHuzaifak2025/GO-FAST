    ...
    const fetchRides = async () => {
        ...
        try {
            const response = await axios.get(
                `${process.env.EXPO_PUBLIC_BACKEND_URL}/gofast/api/ride/history`,
            );
            if (response.status === 200) {
                const list = [
                    ...response.data.data[1].ride,
                    ...response.data.data[1].ride_passenger,
                ];
                setRides(list);
            }
        } catch (error) {
        ...
        <View style={styles.cardContainer}>
            ...
            <View style={styles.cardBody}>
                ...
                <View style={styles.fareContainer}>
                    ...
                    <Text style={styles.fareAmount}>PKR: {item.fare.toFixed(2)}</Text>
                </View>
            </View>
            {item.driver === user_id && (
